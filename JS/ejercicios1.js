...
function esPrimo(n) {
  if (
    ...
    //Se verifica que el numero sea entero, positivo y no 0
...
function convertir(numero, baseInicial, baseFinal) {
  ...
  if (
    numero === undefined ||
    baseInicial === undefined ||
    baseFinal === undefined
  )
    return console.error("No se ingresó uno de los parámetros");
  //Verifica que se lo valores ingresados sean todos números
  if (
    typeof numero !== `number` ||
    typeof baseInicial !== `number` ||
    typeof baseFinal !== `number`
  )
    return console.warn(`Uno de los parámetros no es un número`);
  ...
  if (baseInicial < 2 || baseInicial > 36 || baseFinal < 2 || baseFinal > 36)
    return console.warn(`La base debe comprender entre 2 y 36`);
...
function aplicaDescuento(precio, descuento) {
  ...
  if (typeof precio !== `number` || typeof descuento !== `number`)
    return console.warn("Uno de los valores no es un número");
...
function calculaEdad(fechaN) {
  if (fechaN === undefined)
    return console.error("No se ha introdcido ningún parámetro");
  if (typeof fechaN !== `object`)
    return console.warn("Debe introducir un objeto Date");
...
function calculaEdad2(fechaN) {
  if (fechaN === undefined)
    return console.warn("No ingresaste ningún parámetro");
  if (fechaN instanceof Date !== true)
...
function cuentaVocalesConsonantes(texto) {
  ...
  return console.log(
    `"${texto}" tiene ${arregloNoVocales.length} vocales y ${arregloNoConsonantes.length} consonantes`,
  );
}
...
function nombreValido(nombre) {
  ...
  if (typeof nombre !== `string`)
    return console.warn("El parámetro ingresado no es una cadena de texto");
  if (!nombre) return console.warn("Se ingresó un texto vacio");
  const reLetras = /[^a-záéíóú\s]/gi;
  if (reLetras.test(nombre)) {
    return console.log(
      `"${nombre}" no es un nombre válido, favor de revisar que no contenga carácteres especiales`,
    );
  } else {
...
// pe. miFuncion("correo@gmail.com") devolverá verdadero.
...
function elevaCuadrado(numeros) {
  if (numeros === undefined)
    return console.warn("No ingresaste ningún parámetro");
  if (numeros instanceof Array !== true)
    return console.error("No ingresaste un array");
  if (numeros.length === 0) return console.warn("El arreglo está vacio");
  for (let num of numeros) {
    if (typeof num !== `number`)
      return console.error(
        "Al menos no de los elementos del arreglo no es un número",
      );
  }
...
function numeroMaxMin(numeros) {
  ...
  return console.log(
    `El número mayor es: ${numMax} y el numero menor es: ${numMin}`,
  );
}
...
function separaParesImpares(numeros) {
  ...
  for (let num of numeros) {
    ...
    if (Number.isInteger(num) !== true)
      return console.warn("Al menos un número no es entero");
  }
  ...
  return console.log(
    `Numeros pares: ${numerosSeparados.pares} impares: ${numerosSeparados.impares}`,
  );
}
...
function arreglaAscDesc(arrNum) {
  if (arrNum === undefined)
    return console.warn("No ingresaste ningún parámetro");
  if (arrNum instanceof Array !== true)
    return console.error("No ingresaste un array");
  ...
  validarEstreno(estreno) {
    if (this.validarNumero("Estreno", estreno))
      if (!/^([0-9]{4}$)/.test(estreno))
        return console.error(
          `Año de estreno "${estreno}" no es válido, debe ser un número de 4 caracteres`,
        );
  }
  validarPais(pais) {
